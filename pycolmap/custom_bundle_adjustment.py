...
class PyBundleAdjuster(object):
    ...
    def solve(self, reconstruction: pycolmap.Reconstruction):
        ...
        if self.problem.num_residuals == 0:
            return False
        solver_options = self.set_up_solver_options(self.problem, self.options.solver_options)
    ...
    def set_up_solver_options(self,
    ...
    def add_image_to_problem(self,
        ...
        constant_cam_pose = (not self.options.refine_extrinsics) or self.config.has_constant_cam_pose(image.image_id)
        num_observations = 0
        for point2D in image.points2D:
            ...
            if constant_cam_pose:
                cost = pycolmap.cost_functions.ReprojErrorCost(camera.model, pose, point2D.xy)
                self.problem.add_residual_block(cost, loss, [point3D.xyz, camera.params])
            else:
                cost = pycolmap.cost_functions.ReprojErrorCost(camera.model, point2D.xy)
                self.problem.add_residual_block(cost, loss, [pose.rotation.quat, pose.translation, point3D.xyz, camera.params])
        if num_observations > 0:
            self.camera_ids.add(image.camera_id)
            # Set pose parameterization
            if not constant_cam_pose:
                self.problem.set_manifold(pose.rotation.quat, pyceres.QuaternionManifold())
                if self.config.has_constant_cam_positions(image_id):
                    constant_position_idxs = self.config.constant_cam_positions(image_id)
                    self.problem.set_manifold(pose.translation, pyceres.SubsetManifold(3, constant_position_idxs))


    def add_point_to_problem(self,
                             point3D_id: int,
                             reconstruction: pycolmap.Reconstruction,
                             loss: pyceres.LossFunction):
        ...
        for track_el in point3D.track.elements:
            ...
            cost = pycolmap.cost_functions.ReprojErrorCost(camera.model, image.cam_from_world, point2D.xy)
            self.problem.add_residual_block(cost, loss, [point3D.xyz, camera.params])

    def parameterize_cameras(self, reconstruction: pycolmap.Reconstruction):
        constant_camera = (not self.options.refine_focal_length) and (not self.options.refine_principal_point) and (not options.refine_extra_params)
        for camera_id in self.camera_ids:
            camera = reconstruction.cameras[camera_id]
            if constant_camera or self.config.has_constant_cam_intrinsics(camera_id):
            ...
            if len(const_camera_params) > 0:
                self.problem.set_manifold(camera.params, pyceres.SubsetManifold(len(camera.params), const_camera_params))
