...
# COLMAP overlay: the Visual Studio toolchain bundles an LLVM flang Fortran
# compiler that miscompiles LAPACK's *gedmd routines (e.g. "'ssum' is not an
# object that can appear in an expression"). vcpkg builds each port in its own
# environment (regenerated via vcvars), which puts the bundled LLVM tools on the
# PATH, so vcpkg_find_fortran detects flang instead of falling back to a working
# MinGW gfortran. Drop the bundled LLVM tools directory from the PATH here so the
# MinGW gfortran fallback kicks in. COLMAP itself builds with MSVC (cl.exe) and
...
if(VCPKG_TARGET_IS_WINDOWS)
    cmake_path(CONVERT "$ENV{PATH}" TO_CMAKE_PATH_LIST _lapack_path NORMALIZE)
    list(FILTER _lapack_path EXCLUDE REGEX "[Tt]ools/[Ll]lvm")
    cmake_path(CONVERT "${_lapack_path}" TO_NATIVE_PATH_LIST _lapack_path)
    set(ENV{PATH} "${_lapack_path}")
    unset(_lapack_path)
endif()
