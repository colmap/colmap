...
- ``SIMPLE_EQUIANGULAR``, ``EQUIANGULAR``: Use these camera models for fisheye
  lenses with equiangular projection where distortion can be ignored
  or has been pre-corrected. These models use the fisheye projection
  (theta = atan(r)) without any distortion parameters. ``SIMPLE_EQUIANGULAR``
  has a single focal length (f), while ``EQUIANGULAR`` has two (fx, fy).
