...
- ``SIMPLE_FISHEYE_CAMERA``, ``FISHEYE_CAMERA``: Use these camera models for fisheye
  ...
  (theta = atan(r)) without any distortion parameters. ``SIMPLE_FISHEYE_CAMERA``
  has a single focal length (f), while ``FISHEYE_CAMERA`` has two (fx, fy).
