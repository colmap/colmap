...
bool boost_fusion_tuple_test(std::ostream &log, const char* archive_type) {
	...
	ia & YAS_OBJECT("v2", v2);
	if ( v1 != v2 ) {
		YAS_TEST_REPORT(log, "FUSION_TUPLE deserialization error!");
		return false;
	}
	...
	oa3 & YAS_OBJECT("v", boost::fusion::make_tuple(33,44));
	...
	oa5 & YAS_OBJECT("vv6", std::move(vv6));
	...
	if ( yas::is_binary_archive<typename archive_traits::oarchive_type>::value ) {
		if ( oa6.size() != (archive_traits::oarchive_type::header_size()+1) ) {
			YAS_TEST_REPORT(log, "FUSION_TUPLE serialization error!");
			return false;
		}
	}
