...
struct UndistortCameraOptions {
  ...
  // Whether to enable rectangular ROI(region of interest) in original image.
  bool roi = false;
  // The 4 factors in the range [0, 1] that define the ROI. The bounding box
  // pixel coordinates are calculated as (roi_min_x * Width, roi_min_y * Height)
  // and (roi_max_x * Width, roi_max_y * Height).
