...
__global__ void __launch_bounds__(1024, 1)
    PinholeFocalAndExtra_alpha_numerator_denominator_kernel(
        ...
        double* const PinholeFocalAndExtra_total_ag,
        double* const PinholeFocalAndExtra_total_ac,
        size_t problem_size) {
  const int global_thread_idx = blockIdx.x * blockDim.x + threadIdx.x;
  __shared__ uint8_t inout_shared[256];

  __shared__ double PinholeFocalAndExtra_total_ag_local[1];

  __shared__ double PinholeFocalAndExtra_total_ac_local[1];
  ...
  sum_store<double>(PinholeFocalAndExtra_total_ag_local,
                    (double*)inout_shared,
                    0,
  ...
  sum_store<double>(PinholeFocalAndExtra_total_ac_local,
                    (double*)inout_shared,
                    0,
                    global_thread_idx < problem_size,
                    r3);
