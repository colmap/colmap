...
__global__ void __launch_bounds__(1024, 1)
    simple_radial_fixed_pose_fixed_point_score_kernel(
        double* calib,
  ...
  if (global_thread_idx < problem_size) {
    read_shared_2<double>(
        (double*)inout_shared, calib_indices_loc[threadIdx.x].target, r23, r19);
  };
  __syncthreads();
  if (global_thread_idx < problem_size) {
...
void simple_radial_fixed_pose_fixed_point_score(double* calib,
                                                unsigned int calib_num_alloc,
                                                SharedIndex* calib_indices,
                                                double* pixel,
                                                unsigned int pixel_num_alloc,
  ...
  simple_radial_fixed_pose_fixed_point_score_kernel<<<n_blocks, 1024>>>(
      calib,
      calib_num_alloc,
      calib_indices,
      pixel,
