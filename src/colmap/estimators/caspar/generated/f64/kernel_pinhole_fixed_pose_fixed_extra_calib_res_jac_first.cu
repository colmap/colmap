...
__global__ void __launch_bounds__(1024, 1)
    pinhole_fixed_pose_fixed_extra_calib_res_jac_first_kernel(
        double* focal,
  ...
  if (global_thread_idx < problem_size) {
    read_shared_2<double>(
        (double*)inout_shared, focal_indices_loc[threadIdx.x].target, r5, r26);
  };
  __syncthreads();
  if (global_thread_idx < problem_size) {
    read_idx_2<1024, double, double, double2>(
  ...
  if (global_thread_idx < problem_size) {
    ...
    r20 = r20 * r3;
    r20 = r20 * r0;
    write_sum_2<double, double>((double*)inout_shared, r21, r20);
  };
  ...
  if (global_thread_idx < problem_size) {
    ...
    r17 = r4 * r3;
    r7 = r4 * r2;
    r7 = fma(r27, r7, r22 * r17);
    write_sum_2<double, double>((double*)inout_shared, r0, r7);
  };
...
void pinhole_fixed_pose_fixed_extra_calib_res_jac_first(
    double* focal,
    unsigned int focal_num_alloc,
    SharedIndex* focal_indices,
    double* point,
  ...
  pinhole_fixed_pose_fixed_extra_calib_res_jac_first_kernel<<<n_blocks, 1024>>>(
      focal,
      focal_num_alloc,
      focal_indices,
      point,
