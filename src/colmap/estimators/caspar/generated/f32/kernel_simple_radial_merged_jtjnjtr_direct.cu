...
__global__ void __launch_bounds__(1024, 1)
    SimpleRadialMergedJtjnjtrDirectKernel(float* pose_njtr,
  ...
  if (global_thread_idx < problem_size) {
    ...
    ReadIdx4<1024, float, float, float4>(
        pose_jac, 4 * pose_jac_num_alloc, global_thread_idx, r7, r23, r24, r25);
  ...
  if (global_thread_idx < problem_size) {
    ReadIdx4<1024, float, float, float4>(
        pose_jac, 8 * pose_jac_num_alloc, global_thread_idx, r27, r26, r5, r14);
...
void SimpleRadialMergedJtjnjtrDirect(float* pose_njtr,
                                     unsigned int pose_njtr_num_alloc,
                                     SharedIndex* pose_njtr_indices,
                                     float* pose_jac,
                                     unsigned int pose_jac_num_alloc,
  ...
  SimpleRadialMergedJtjnjtrDirectKernel<<<n_blocks, 1024>>>(
      pose_njtr,
      pose_njtr_num_alloc,
      pose_njtr_indices,
      pose_jac,
