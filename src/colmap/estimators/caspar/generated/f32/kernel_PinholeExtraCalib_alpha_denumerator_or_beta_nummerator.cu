...
__global__ void __launch_bounds__(1024, 1)
    PinholeExtraCalib_alpha_denumerator_or_beta_nummerator_kernel(
        ...
        float* const PinholeExtraCalib_out,
        size_t problem_size) {
  const int global_thread_idx = blockIdx.x * blockDim.x + threadIdx.x;
  __shared__ uint8_t inout_shared[128];

  __shared__ float PinholeExtraCalib_out_local[1];
  ...
  sum_store<float>(PinholeExtraCalib_out_local,
                   (float*)inout_shared,
                   0,
                   global_thread_idx < problem_size,
                   r2);
  sum_flush_final<float>(PinholeExtraCalib_out_local, PinholeExtraCalib_out, 1);
}
