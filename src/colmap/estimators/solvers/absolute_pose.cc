...
// The manifold of a cam_from_world transform: rotation on SO(3), with the
// translation as Euclidean parameters. The ambient parameter layout matches
// TinyPnPCostFunctor: [qx, qy, qz, qw, tx, ty, tz].
using CamFromWorldManifold =
...
using PnFManifold = ProductManifold<CamFromWorldManifold,
                                    EuclideanManifold<kSharedFocal ? 1 : 2>>;
...
template <bool kSharedFocal>
bool RefinePnFPoseWithTinySolver(const std::vector<Eigen::Vector2d>& points2D,
                                 const std::vector<Eigen::Vector3d>& points3D,
  ...
  using Solver = TinySolver<decltype(f), PnFManifold<kSharedFocal>>;
  Solver solver;
  ...
  model->focal_lengths.y() = kSharedFocal ? std::exp(x[7]) : std::exp(x[8]);
  return true;
}
...
bool P3PEstimator::Refine(const std::vector<X_t>& points2D,
  ...
  using Solver = TinySolver<decltype(f), CamFromWorldManifold>;
  Solver solver;
  typename Solver::Options options;
  options.max_num_iterations = 25;
...
bool P4PFEstimator::Refine(const std::vector<X_t>& points2D,
  ...
  if (share_focal_length_) {
    return RefinePnFPoseWithTinySolver<true>(points2D, points3D, model);
  } else {
    return RefinePnFPoseWithTinySolver<false>(points2D, points3D, model);
  }
}
