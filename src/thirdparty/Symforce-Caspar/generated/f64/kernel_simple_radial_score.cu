...
__global__ void __launch_bounds__(1024, 1) SimpleRadialScoreKernel(
  ...
  if (global_thread_idx < problem_size) {
    ReadShared2<double>((double *)inout_shared,
                        calib_indices_loc[threadIdx.x].target, r23, r18);
  };
  __syncthreads();
  if (global_thread_idx < problem_size) {
...
void SimpleRadialScore(double *pose, unsigned int pose_num_alloc,
  ...
  SimpleRadialScoreKernel<<<n_blocks, 1024>>>(
      ...
      point, point_num_alloc, point_indices, pixel, pixel_num_alloc, out_rTr,
      problem_size);
}
