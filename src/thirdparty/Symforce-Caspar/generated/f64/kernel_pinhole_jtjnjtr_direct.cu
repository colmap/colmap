...
__global__ void __launch_bounds__(1024, 1) PinholeJtjnjtrDirectKernel(
  ...
  if (global_thread_idx < problem_size) {
    ReadShared2<double>((double *)inout_shared,
                        calib_njtr_indices_loc[threadIdx.x].target, r12, r13);
  };
  __syncthreads();
  ...
  if (global_thread_idx < problem_size) {
    ...
    WriteSum2<double, double>((double *)inout_shared, r23, r12);
  };
  ...
  if (global_thread_idx < problem_size) {
    ReadShared2<double>((double *)inout_shared,
                        pose_njtr_indices_loc[threadIdx.x].target, r12, r23);
  };
  __syncthreads();
  ...
  if (global_thread_idx < problem_size) {
    ReadShared2<double>((double *)inout_shared,
                        pose_njtr_indices_loc[threadIdx.x].target, r13, r25);
  };
  __syncthreads();
  if (global_thread_idx < problem_size) {
