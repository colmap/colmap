...
__global__ void __launch_bounds__(1024, 1)
    ThinPrismFisheyeSplitFixedFocalAndExtraFixedPrincipalPointScoreKernel(
        ...
        double *pixel, unsigned int pixel_num_alloc, double *const out_rTr,
        size_t problem_size) {
  ...
  if (global_thread_idx < problem_size) {
    ReadShared2<double>((double *)inout_shared,
                        pose_indices_loc[threadIdx.x].target, r7, r28);
  };
  __syncthreads();
  if (global_thread_idx < problem_size) {
    r29 = r16 * r12;
...
void ThinPrismFisheyeSplitFixedFocalAndExtraFixedPrincipalPointScore(
  ...
  ThinPrismFisheyeSplitFixedFocalAndExtraFixedPrincipalPointScoreKernel<<<
      ...
      point_indices, pixel, pixel_num_alloc, out_rTr, problem_size);
}
