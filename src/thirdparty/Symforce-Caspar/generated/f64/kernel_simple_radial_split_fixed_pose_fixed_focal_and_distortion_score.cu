...
__global__ void __launch_bounds__(1024, 1)
    SimpleRadialSplitFixedPoseFixedFocalAndDistortionScoreKernel(
        ...
        unsigned int focal_and_distortion_num_alloc, double *const out_rTr,
        size_t problem_size) {
  ...
  if (global_thread_idx < problem_size) {
    ReadShared1<double>((double *)inout_shared,
                        point_indices_loc[threadIdx.x].target, r0);
  };
  __syncthreads();
  if (global_thread_idx < problem_size) {
    r18 = r13 * r9;
...
void SimpleRadialSplitFixedPoseFixedFocalAndDistortionScore(
    double *principal_point, unsigned int principal_point_num_alloc,
    SharedIndex *principal_point_indices, double *point,
    unsigned int point_num_alloc, SharedIndex *point_indices, double *pixel,
    unsigned int pixel_num_alloc, double *pose, unsigned int pose_num_alloc,
  ...
  SimpleRadialSplitFixedPoseFixedFocalAndDistortionScoreKernel<<<n_blocks,
                                                                 1024>>>(
