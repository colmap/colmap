...
__global__ void __launch_bounds__(1024, 1) PinholeCalibUpdateStepKernel(
  ...
  if (global_thread_idx < problem_size) {
    ...
    WriteIdx2<1024, double, double, double2>(
        out_PinholeCalib_step_kp1,
        2 * out_PinholeCalib_step_kp1_num_alloc,
        global_thread_idx,
        r1,
        r4);
  };
...
void PinholeCalibUpdateStep(double* PinholeCalib_step_k,
                            unsigned int PinholeCalib_step_k_num_alloc,
                            double* PinholeCalib_p_kp1,
                            unsigned int PinholeCalib_p_kp1_num_alloc,
                            const double* const alpha,
  ...
  PinholeCalibUpdateStepKernel<<<n_blocks, 1024>>>(
      PinholeCalib_step_k,
      PinholeCalib_step_k_num_alloc,
      PinholeCalib_p_kp1,
      PinholeCalib_p_kp1_num_alloc,
      alpha,
      out_PinholeCalib_step_kp1,
      out_PinholeCalib_step_kp1_num_alloc,
      problem_size);
}
