...
__global__ void __launch_bounds__(1024, 1)
    PointAlphaDenominatorOrBetaNumeratorKernel(
        ...
        double *const Point_out, size_t problem_size) {
  ...
  SumStore<double>(Point_out_local, (double *)inout_shared, 0,
                   global_thread_idx < problem_size, r2);
  SumFlushFinal<double>(Point_out_local, Point_out, 1);
}
...
void PointAlphaDenominatorOrBetaNumerator(double *Point_p_kp1,
  ...
  PointAlphaDenominatorOrBetaNumeratorKernel<<<n_blocks, 1024>>>(
      Point_p_kp1, Point_p_kp1_num_alloc, Point_w, Point_w_num_alloc, Point_out,
      problem_size);
}
