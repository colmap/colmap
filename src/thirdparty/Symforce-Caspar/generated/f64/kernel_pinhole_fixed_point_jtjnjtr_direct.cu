...
__global__ void __launch_bounds__(1024, 1) PinholeFixedPointJtjnjtrDirectKernel(
  ...
  if (global_thread_idx < problem_size) {
    ...
    ReadIdx2<1024, double, double, double2>(pose_jac, 8 * pose_jac_num_alloc,
                                            global_thread_idx, r10, r14);
    r4 = fma(r10, r4, r14 * r5);
    WriteSum2<double, double>((double *)inout_shared, r13, r4);
  };
  ...
  if (global_thread_idx < problem_size) {
    ReadShared2<double>((double *)inout_shared,
                        pose_njtr_indices_loc[threadIdx.x].target, r4, r13);
  };
  __syncthreads();
  ...
  if (global_thread_idx < problem_size) {
    ReadShared2<double>((double *)inout_shared,
                        pose_njtr_indices_loc[threadIdx.x].target, r5, r15);
  };
  __syncthreads();
  if (global_thread_idx < problem_size) {
  ...
  if (global_thread_idx < problem_size) {
    ReadShared2<double>((double *)inout_shared,
                        pose_njtr_indices_loc[threadIdx.x].target, r10, r16);
  };
  __syncthreads();
  if (global_thread_idx < problem_size) {
    r9 = fma(r15, r11, r9);
  ...
  FlushSumShared<2, double>(out_calib_njtr, 0 * out_calib_njtr_num_alloc,
                            calib_njtr_indices_loc, (double *)inout_shared);
  if (global_thread_idx < problem_size) {
    WriteSum2<double, double>((double *)inout_shared, r9, r3);
  };
