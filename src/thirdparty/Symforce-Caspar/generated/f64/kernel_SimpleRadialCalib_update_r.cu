...
__global__ void __launch_bounds__(1024, 1) SimpleRadialCalibUpdateRKernel(
    ...
    const double *const negalpha, double *out_SimpleRadialCalib_r_kp1,
    unsigned int out_SimpleRadialCalib_r_kp1_num_alloc,
    double *const out_SimpleRadialCalib_r_kp1_norm2_tot, size_t problem_size) {
  ...
  if (global_thread_idx < problem_size) {
    ...
    r4 = fma(r6, r4, r0);
    WriteIdx2<1024, double, double, double2>(
        out_SimpleRadialCalib_r_kp1, 2 * out_SimpleRadialCalib_r_kp1_num_alloc,
        global_thread_idx, r5, r4);
    r2 = fma(r2, r2, r4 * r4);
    r2 = fma(r5, r5, r2);
    r2 = fma(r3, r3, r2);
  };
  SumStore<double>(out_SimpleRadialCalib_r_kp1_norm2_tot_local,
                   (double *)inout_shared, 0, global_thread_idx < problem_size,
                   r2);
  SumFlushFinal<double>(out_SimpleRadialCalib_r_kp1_norm2_tot_local,
                        out_SimpleRadialCalib_r_kp1_norm2_tot, 1);
}

void SimpleRadialCalibUpdateR(
    double *SimpleRadialCalib_r_k, unsigned int SimpleRadialCalib_r_k_num_alloc,
    double *SimpleRadialCalib_w, unsigned int SimpleRadialCalib_w_num_alloc,
    const double *const negalpha, double *out_SimpleRadialCalib_r_kp1,
    unsigned int out_SimpleRadialCalib_r_kp1_num_alloc,
  ...
  SimpleRadialCalibUpdateRKernel<<<n_blocks, 1024>>>(
