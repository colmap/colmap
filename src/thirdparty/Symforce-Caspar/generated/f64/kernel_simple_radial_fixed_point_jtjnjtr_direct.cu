...
__global__ void __launch_bounds__(1024, 1)
    SimpleRadialFixedPointJtjnjtrDirectKernel(
        ...
        double *calib_jac, unsigned int calib_jac_num_alloc,
  ...
  if (global_thread_idx < problem_size) {
    ...
    WriteSum2<double, double>((double *)inout_shared, r5, r11);
  };
  ...
  if (global_thread_idx < problem_size) {
    ...
    WriteSum2<double, double>((double *)inout_shared, r16, r2);
  };
  ...
  if (global_thread_idx < problem_size) {
    ReadShared2<double>((double *)inout_shared,
                        pose_njtr_indices_loc[threadIdx.x].target, r2, r16);
  };
  __syncthreads();
  if (global_thread_idx < problem_size) {
  ...
  if (global_thread_idx < problem_size) {
    ReadShared2<double>((double *)inout_shared,
                        pose_njtr_indices_loc[threadIdx.x].target, r17, r4);
  };
  __syncthreads();
  ...
  if (global_thread_idx < problem_size) {
    ReadShared2<double>((double *)inout_shared,
                        pose_njtr_indices_loc[threadIdx.x].target, r19, r20);
  };
  __syncthreads();
  if (global_thread_idx < problem_size) {
    r15 = fma(r17, r11, r15);
  ...
  FlushSumShared<2, double>(out_calib_njtr, 0 * out_calib_njtr_num_alloc,
                            calib_njtr_indices_loc, (double *)inout_shared);
  if (global_thread_idx < problem_size) {
    WriteSum2<double, double>((double *)inout_shared, r15, r14);
  };
  FlushSumShared<2, double>(out_calib_njtr, 2 * out_calib_njtr_num_alloc,
                            calib_njtr_indices_loc, (double *)inout_shared);
}

void SimpleRadialFixedPointJtjnjtrDirect(
    double *pose_njtr, unsigned int pose_njtr_num_alloc,
    SharedIndex *pose_njtr_indices, double *pose_jac,
    unsigned int pose_jac_num_alloc, double *calib_njtr,
    unsigned int calib_njtr_num_alloc, SharedIndex *calib_njtr_indices,
  ...
  SimpleRadialFixedPointJtjnjtrDirectKernel<<<n_blocks, 1024>>>(
