...
__global__ void __launch_bounds__(1024, 1)
    SimpleRadialSplitFixedPrincipalPointJtjnjtrDirectKernel(
        ...
        unsigned int focal_and_distortion_jac_num_alloc, double *point_njtr,
  ...
  if (global_thread_idx < problem_size) {
    ReadShared2<double>(
        (double *)inout_shared,
        focal_and_distortion_njtr_indices_loc[threadIdx.x].target, r12, r13);
  };
  __syncthreads();
  if (global_thread_idx < problem_size) {
  ...
  if (global_thread_idx < problem_size) {
    ReadShared2<double>((double *)inout_shared,
                        pose_njtr_indices_loc[threadIdx.x].target, r12, r26);
  };
  __syncthreads();
  if (global_thread_idx < problem_size) {
    r25 = fma(r12, r25, r26 * r27);
  };
  LoadShared<2, double, double>(pose_njtr, 2 * pose_njtr_num_alloc,
                                pose_njtr_indices_loc, (double *)inout_shared);
  if (global_thread_idx < problem_size) {
    ReadShared2<double>((double *)inout_shared,
                        pose_njtr_indices_loc[threadIdx.x].target, r27, r19);
  };
  __syncthreads();
...
void SimpleRadialSplitFixedPrincipalPointJtjnjtrDirect(
  ...
  SimpleRadialSplitFixedPrincipalPointJtjnjtrDirectKernel<<<n_blocks, 1024>>>(
      ...
      focal_and_distortion_jac, focal_and_distortion_jac_num_alloc, point_njtr,
      point_njtr_num_alloc, point_njtr_indices, point_jac, point_jac_num_alloc,
      out_pose_njtr, out_pose_njtr_num_alloc, out_focal_and_distortion_njtr,
      out_focal_and_distortion_njtr_num_alloc, out_point_njtr,
