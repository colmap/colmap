...
__global__ void __launch_bounds__(1024, 1)
    PinholePosePredDecreaseTimesTwoKernel(
        double *PinholePose_step, unsigned int PinholePose_step_num_alloc,
        double *PinholePose_precond_diag,
        unsigned int PinholePose_precond_diag_num_alloc,
        const double *const diag, double *PinholePose_njtr,
        unsigned int PinholePose_njtr_num_alloc,
        double *const out_PinholePose_pred_dec, size_t problem_size) {
  ...
  SumStore<double>(out_PinholePose_pred_dec_local, (double *)inout_shared, 0,
                   global_thread_idx < problem_size, r13);
  SumFlushFinal<double>(out_PinholePose_pred_dec_local,
                        out_PinholePose_pred_dec, 1);
}
...
void PinholePosePredDecreaseTimesTwo(
    ...
    double *PinholePose_njtr, unsigned int PinholePose_njtr_num_alloc,
    double *const out_PinholePose_pred_dec, size_t problem_size) {
