...
__global__ void __launch_bounds__(1024, 1)
  ...
  if (global_thread_idx < problem_size) {
    ...
    ReadIdx2<1024, double, double, double2>(calib_jac, 2 * calib_jac_num_alloc,
                                            global_thread_idx, r2, r12);
    r13 = fma(r2, r6, r12 * r9);
  ...
  if (global_thread_idx < problem_size) {
    ReadShared2<double>((double *)inout_shared,
                        calib_njtr_indices_loc[threadIdx.x].target, r9, r6);
  };
  __syncthreads();
  ...
  if (global_thread_idx < problem_size) {
    ReadShared2<double>((double *)inout_shared,
                        calib_njtr_indices_loc[threadIdx.x].target, r13, r5);
  };
  __syncthreads();
  if (global_thread_idx < problem_size) {
    r1 = fma(r13, r1, r6);
...
void SimpleRadialFixedPoseJtjnjtrDirect(
    double *calib_njtr, unsigned int calib_njtr_num_alloc,
    SharedIndex *calib_njtr_indices, double *calib_jac,
    unsigned int calib_jac_num_alloc, double *point_njtr,
    unsigned int point_njtr_num_alloc, SharedIndex *point_njtr_indices,
  ...
  SimpleRadialFixedPoseJtjnjtrDirectKernel<<<n_blocks, 1024>>>(
