...
__global__ void __launch_bounds__(1024, 1)
    SimpleRadialFocalAndDistortionAlphaDenominatorOrBetaNumeratorKernel(
        ...
        double *const SimpleRadialFocalAndDistortion_out, size_t problem_size) {
  const int global_thread_idx = blockIdx.x * blockDim.x + threadIdx.x;
  __shared__ uint8_t inout_shared[256];

  __shared__ double SimpleRadialFocalAndDistortion_out_local[1];
  ...
  SumStore<double>(SimpleRadialFocalAndDistortion_out_local,
                   (double *)inout_shared, 0, global_thread_idx < problem_size,
                   r3);
  SumFlushFinal<double>(SimpleRadialFocalAndDistortion_out_local,
                        SimpleRadialFocalAndDistortion_out, 1);
}
...
void SimpleRadialFocalAndDistortionAlphaDenominatorOrBetaNumerator(
  ...
  SimpleRadialFocalAndDistortionAlphaDenominatorOrBetaNumeratorKernel<<<
      n_blocks, 1024>>>(SimpleRadialFocalAndDistortion_p_kp1,
                        SimpleRadialFocalAndDistortion_p_kp1_num_alloc,
                        SimpleRadialFocalAndDistortion_w,
                        SimpleRadialFocalAndDistortion_w_num_alloc,
                        SimpleRadialFocalAndDistortion_out, problem_size);
}
