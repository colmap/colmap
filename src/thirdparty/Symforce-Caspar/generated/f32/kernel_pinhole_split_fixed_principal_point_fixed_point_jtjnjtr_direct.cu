...
__global__ void __launch_bounds__(1024, 1)
  ...
  if (global_thread_idx < problem_size) {
    ...
    ReadIdx2<1024, float, float, float2>(pose_jac, 8 * pose_jac_num_alloc,
                                         global_thread_idx, r15, r14);
    r5 = fmaf(r14, r5, r15 * r4);
    WriteSum2<float, float>((float *)inout_shared, r8, r5);
  };
  ...
  if (global_thread_idx < problem_size) {
    ReadShared2<float>((float *)inout_shared,
                       pose_njtr_indices_loc[threadIdx.x].target, r5, r8);
  };
  __syncthreads();
  ...
  if (global_thread_idx < problem_size) {
    ReadShared4<float>((float *)inout_shared,
                       pose_njtr_indices_loc[threadIdx.x].target, r4, r13, r16,
                       r17);
  };
  __syncthreads();
  if (global_thread_idx < problem_size) {
