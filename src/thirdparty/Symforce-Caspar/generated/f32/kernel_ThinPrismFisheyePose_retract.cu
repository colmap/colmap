...
__global__ void __launch_bounds__(1024, 1) ThinPrismFisheyePoseRetractKernel(
  ...
  if (global_thread_idx < problem_size) {
    ...
    WriteIdx4<1024, float, float, float4>(
        ...
        0 * out_ThinPrismFisheyePose_retracted_num_alloc, global_thread_idx,
        r12, r14, r15, r9);
    ReadIdx3<1024, float, float, float4>(ThinPrismFisheyePose,
                                         4 * ThinPrismFisheyePose_num_alloc,
                                         global_thread_idx, r9, r15, r14);
    r8 = r9 + r8;
    ReadIdx2<1024, float, float, float2>(delta, 4 * delta_num_alloc,
                                         global_thread_idx, r9, r12);
    r9 = r15 + r9;
    r12 = r14 + r12;
    WriteIdx3<1024, float, float, float4>(
        out_ThinPrismFisheyePose_retracted,
        4 * out_ThinPrismFisheyePose_retracted_num_alloc, global_thread_idx, r8,
        r9, r12);
  };
}
