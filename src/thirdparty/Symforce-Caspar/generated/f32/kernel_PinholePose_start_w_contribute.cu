...
__global__ void __launch_bounds__(1024, 1) PinholePoseStartWContributeKernel(
    ...
    float *PinholePose_p, unsigned int PinholePose_p_num_alloc,
    float *out_PinholePose_w, unsigned int out_PinholePose_w_num_alloc,
    size_t problem_size) {
  ...
  if (global_thread_idx < problem_size) {
    r0 = r0 * r4;
    ReadIdx4<1024, float, float, float4>(PinholePose_p,
    ...
    r3 = r3 * r4;
    r3 = r3 * r8;
    AddIdx4<1024, float, float, float4>(out_PinholePose_w,
    ...
    r4 = r2 * r4;
    r4 = r4 * r0;
    AddIdx2<1024, float, float, float2>(out_PinholePose_w,
