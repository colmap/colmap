...
__global__ void __launch_bounds__(1024, 1)
    SimpleRadialSplitFixedFocalAndDistortionScoreKernel(
        ...
        unsigned int focal_and_distortion_num_alloc, float *const out_rTr,
        size_t problem_size) {
  ...
  if (global_thread_idx < problem_size) {
    ReadShared3<float>((float *)inout_shared,
                       pose_indices_loc[threadIdx.x].target, r0, r5, r6);
  };
  __syncthreads();
...
void SimpleRadialSplitFixedFocalAndDistortionScore(
    float *pose, unsigned int pose_num_alloc, SharedIndex *pose_indices,
    float *principal_point, unsigned int principal_point_num_alloc,
    SharedIndex *principal_point_indices, float *point,
    unsigned int point_num_alloc, SharedIndex *point_indices, float *pixel,
  ...
  SimpleRadialSplitFixedFocalAndDistortionScoreKernel<<<n_blocks, 1024>>>(
      ...
      focal_and_distortion, focal_and_distortion_num_alloc, out_rTr,
      problem_size);
}
