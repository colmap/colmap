...
__global__ void __launch_bounds__(1024, 1)
    PinholePrincipalPointUpdateRFirstKernel(
        ...
        const float *const negalpha, float *out_PinholePrincipalPoint_r_kp1,
  ...
  SumStore<float>(out_PinholePrincipalPoint_r_0_norm2_tot_local,
                  (float *)inout_shared, 0, global_thread_idx < problem_size,
                  r0);
  if (global_thread_idx < problem_size) {
    r2 = fmaf(r2, r2, r4 * r4);
  };
  SumStore<float>(out_PinholePrincipalPoint_r_kp1_norm2_tot_local,
                  (float *)inout_shared, 0, global_thread_idx < problem_size,
                  r2);
  SumFlushFinal<float>(out_PinholePrincipalPoint_r_0_norm2_tot_local,
                       out_PinholePrincipalPoint_r_0_norm2_tot, 1);
  SumFlushFinal<float>(out_PinholePrincipalPoint_r_kp1_norm2_tot_local,
...
void PinholePrincipalPointUpdateRFirst(
    ...
    unsigned int PinholePrincipalPoint_r_k_num_alloc,
    float *PinholePrincipalPoint_w,
    unsigned int PinholePrincipalPoint_w_num_alloc, const float *const negalpha,
