...
__global__ void __launch_bounds__(1024, 1)
    ThinPrismFisheyeSplitFixedPoseFixedFocalAndExtraFixedPrincipalPointScoreKernel(
        ...
        float *pose, unsigned int pose_num_alloc, float *focal_and_extra,
        unsigned int focal_and_extra_num_alloc, float *principal_point,
        unsigned int principal_point_num_alloc, float *pixel,
  ...
  if (global_thread_idx < problem_size) {
    ReadShared3<float>((float *)inout_shared,
                       point_indices_loc[threadIdx.x].target, r14, r15, r16);
  };
  __syncthreads();
  if (global_thread_idx < problem_size) {
    ...
    ReadIdx4<1024, float, float, float4>(focal_and_extra,
                                         ...
                                         global_thread_idx, r35, r41, r34, r14);
...
void ThinPrismFisheyeSplitFixedPoseFixedFocalAndExtraFixedPrincipalPointScore(
    float *point, unsigned int point_num_alloc, SharedIndex *point_indices,
    float *sensor_from_rig, unsigned int sensor_from_rig_num_alloc, float *pose,
  ...
  ThinPrismFisheyeSplitFixedPoseFixedFocalAndExtraFixedPrincipalPointScoreKernel<<<
      n_blocks, 1024>>>(point, point_num_alloc, point_indices, sensor_from_rig,
                        sensor_from_rig_num_alloc, pose, pose_num_alloc,
                        focal_and_extra, focal_and_extra_num_alloc,
                        principal_point, principal_point_num_alloc, pixel,
