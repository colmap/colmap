...
__global__ void __launch_bounds__(1024, 1)
    SimpleRadialCalibAlphaNumeratorDenominatorKernel(
        ...
        float *const SimpleRadialCalib_total_ac, size_t problem_size) {
  ...
  SumStore<float>(SimpleRadialCalib_total_ag_local, (float *)inout_shared, 0,
                  global_thread_idx < problem_size, r5);
  if (global_thread_idx < problem_size) {
    ReadIdx4<1024, float, float, float4>(SimpleRadialCalib_w,
                                         0 * SimpleRadialCalib_w_num_alloc,
                                         global_thread_idx, r5, r4, r7, r6);
    r7 = fmaf(r2, r7, r1 * r4);
    r7 = fmaf(r0, r5, r7);
    r7 = fmaf(r3, r6, r7);
  };
  SumStore<float>(SimpleRadialCalib_total_ac_local, (float *)inout_shared, 0,
                  global_thread_idx < problem_size, r7);
  SumFlushFinal<float>(SimpleRadialCalib_total_ag_local,
                       SimpleRadialCalib_total_ag, 1);
  SumFlushFinal<float>(SimpleRadialCalib_total_ac_local,
