...
__global__ void __launch_bounds__(1024, 1)
  ...
  if (global_thread_idx < problem_size) {
    ...
    WriteSum4<float, float>((float *)inout_shared, r5, r7, r16, r17);
  };
  ...
  if (global_thread_idx < problem_size) {
    ReadIdx4<1024, float, float, float4>(pose_jac, 8 * pose_jac_num_alloc,
                                         global_thread_idx, r17, r16, r7, r5);
    r18 = fmaf(r16, r4, r17 * r6);
    r6 = fmaf(r7, r6, r5 * r4);
    WriteSum2<float, float>((float *)inout_shared, r18, r6);
  };
  ...
  if (global_thread_idx < problem_size) {
    ReadShared2<float>((float *)inout_shared,
                       pose_njtr_indices_loc[threadIdx.x].target, r6, r18);
  };
  __syncthreads();
  if (global_thread_idx < problem_size) {
  ...
  if (global_thread_idx < problem_size) {
    ReadShared4<float>((float *)inout_shared,
                       pose_njtr_indices_loc[threadIdx.x].target, r7, r4, r19,
                       r20);
  };
  __syncthreads();
  if (global_thread_idx < problem_size) {
    ...
    WriteSum4<float, float>((float *)inout_shared, r9, r11, r17, r15);
  };
