...
__global__ void __launch_bounds__(1024, 1)
    PinholeSplitFixedPoseFixedPrincipalPointJtjnjtrDirectKernel(
        float *focal_njtr, unsigned int focal_njtr_num_alloc,
        SharedIndex *focal_njtr_indices, float *focal_jac,
        unsigned int focal_jac_num_alloc, float *point_njtr,
  ...
  if (global_thread_idx < problem_size) {
    ReadShared3<float>((float *)inout_shared,
                       point_njtr_indices_loc[threadIdx.x].target, r2, r3, r4);
  };
  __syncthreads();
  if (global_thread_idx < problem_size) {
    ...
    r3 = fmaf(r2, r8, r3);
    r3 = r1 * r3;
    WriteSum2<float, float>((float *)inout_shared, r11, r3);
  };
  ...
  if (global_thread_idx < problem_size) {
    ReadShared2<float>((float *)inout_shared,
                       focal_njtr_indices_loc[threadIdx.x].target, r3, r11);
  };
  __syncthreads();
  if (global_thread_idx < problem_size) {
    r1 = r11 * r1;
    ...
    r0 = fmaf(r5, r0, r6 * r1);
    WriteSum3<float, float>((float *)inout_shared, r7, r9, r0);
  };
