...
__global__ void __launch_bounds__(1024, 1) SphericalPoseUpdateRFirstKernel(
    ...
    float* const out_SphericalPose_r_0_norm2_tot,
    float* const out_SphericalPose_r_kp1_norm2_tot,
    size_t problem_size) {
  const int global_thread_idx = blockIdx.x * blockDim.x + threadIdx.x;
  __shared__ uint8_t inout_shared[4096];

  __shared__ float out_SphericalPose_r_0_norm2_tot_local[1];
  ...
  SumStore<float>(out_SphericalPose_r_0_norm2_tot_local,
                  (float*)inout_shared,
                  0,
  ...
  SumStore<float>(out_SphericalPose_r_kp1_norm2_tot_local,
                  ...
                  global_thread_idx < problem_size,
                  r11);
  SumFlushFinal<float>(out_SphericalPose_r_0_norm2_tot_local,
...
void SphericalPoseUpdateRFirst(float* SphericalPose_r_k,
                               unsigned int SphericalPose_r_k_num_alloc,
                               float* SphericalPose_w,
                               unsigned int SphericalPose_w_num_alloc,
                               const float* const negalpha,
