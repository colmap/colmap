...
__global__ void __launch_bounds__(1024, 1)
    PointUpdatePKernel(float* Point_z,
  ...
  if (global_thread_idx < problem_size) {
    ReadIdx3<1024, float, float, float4>(
        Point_p_k, 0 * Point_p_k_num_alloc, global_thread_idx, r0, r1, r2);
    ReadIdx3<1024, float, float, float4>(
        Point_z, 0 * Point_z_num_alloc, global_thread_idx, r3, r4, r5);
  };
...
void PointUpdateP(float* Point_z,
                  unsigned int Point_z_num_alloc,
                  float* Point_p_k,
                  unsigned int Point_p_k_num_alloc,
                  const float* const beta,
  ...
  PointUpdatePKernel<<<n_blocks, 1024>>>(Point_z,
                                         Point_z_num_alloc,
                                         Point_p_k,
                                         Point_p_k_num_alloc,
                                         beta,
