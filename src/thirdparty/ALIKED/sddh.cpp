...
SDDH::forward(const torch::Tensor& x, const std::vector<torch::Tensor>& keypoints) {
    ...
    for (int64_t batch_idx = 0; batch_idx < batch_size; ++batch_idx)
    {
        ...
        if (num_keypoints == 0) {
          offsets.push_back(torch::zeros({0, 2}, torch::TensorOptions().device(device).dtype(xi.dtype())));
          descriptors.push_back(torch::zeros({0, 128}, torch::TensorOptions().device(device).dtype(xi.dtype())));
          continue;
        }
        ...
        auto offset = offset_conv_->forward(patch);
