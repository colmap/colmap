...
void BindBitmap(pybind11::module& m) {
  py::class_<Bitmap>(m, "Bitmap")
      .def(py::init<>())
      .def("to_array", [](const Bitmap& self) {
        std::vector<ssize_t> shape = {
          static_cast<ssize_t>(self.Height()),
          static_cast<ssize_t>(self.Width())};
          const auto channels = static_cast<ssize_t>(self.Channels());
          const bool is_rgb = self.IsRGB();
        if (channels != 1) {
          if (channels != 3) {
            throw std::runtime_error("Can only convert grayscale or 3-channel RGB image to array");
          }
        ...
        const size_t output_pitch = (output_into.shape.size() == 2) ? output_into.shape[1] : (output_into.shape[1] * output_into.shape[2]);
      ...
      .def_static("from_array", [](py::array_t<uint8_t, py::array::c_style> array) -> Bitmap {
            int channels = 1;
            if (array.ndim() == 3) {
            ...
            for (int y = 0; y < height; ++y) {
              if (is_rgb) {
                for (int x = 0; x < width; ++x) {
                  output.SetPixel(x, y, BitmapColor<uint8_t> (
                ...
                // Copy (guaranteed contiguous) row memory directly.
                std::memcpy(
                    const_cast<uint8_t*>(output.GetScanline(y)),
                    input_row_ptr,
                    width);
              }
      ...
      .def("write", &Bitmap::Write, "path"_a, "flags"_a = 0, "Write bitmap to file.").def("__repr__", &CreateRepresentation<Bitmap>)
      .def_static("read", [](const std::string& path, bool as_rgb) {
        Bitmap bitmap;
        bitmap.Read(path, as_rgb);
        return bitmap;
      }, "path"_a, "as_rgb"_a, "Read bitmap from file.");
}
