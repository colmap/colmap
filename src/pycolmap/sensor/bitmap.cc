  ...
  for (ssize_t y = 0; y < output_into.shape[0]; ++y) {
    if (is_rgb) {
      for (ssize_t x = 0; x < output_into.shape[1]; ++x) {
        ...
        // different striding, so we have to set each pixel separately.
      ...
      std::memcpy(output_row_ptr, self.GetScanline(y), output_into.shape[1]);
    }
...
Bitmap BitmapFromArray(py::array_t<uint8_t, py::array::c_style> array) {
  ...
  if (width == 0 || height == 0) {
    throw std::runtime_error("Input array must have positive width and height");
  }

  if (channels != 1 && channels != 3 && channels != 4) {
    throw std::runtime_error("Input array must have 1, 3, or 4 channels!");
  }
  ...
  for (int y = 0; y < height; ++y) {
    if (is_rgb) {
      ...
      // Copy (guaranteed contiguous) row memory directly.
      std::memcpy(
          const_cast<uint8_t*>(output.GetScanline(y)), input_row_ptr, width);
    }
